...
// Эти обратные вызовы используются только при беспроводной активации, поэтому они
// оставить здесь пустым (мы не можем их полностью опустить, если только
// DISABLE_JOIN устанавливается в arduino-lmoc/project_config/lmic_project_config.h,
// иначе компоновщик будет жаловаться).
